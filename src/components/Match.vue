...
export default {
    props: ['matchId'],
    components:{
      'MatchDetailTable':MatchDetailTable
    },
    created(){
      this.getMatchData();
    },
    ...
    computed:{
        // radiantPlayers(){
        //   let players = this.matchData.players.slice(0,5);
        // //  return 
        // },
        // direPlayers(){
        //   return this.matchData.players.slice(5);
        // },
        expandedMatchId(){
          return this.$store.state.expandedMatchId;
        },
        duration(){
          let seconds = this.matchData.duration
    ...
    methods: {
      getMatchData(){
        this.$axios.get('https://api.opendota.com/api/matches/' + this.matchId).then(
          (response) => {
            // this place need to be optimized first.
            this.matchData = response.data;
            console.log(this.matchId);
            if(this.expandedMatchId != this.matchId){
              this.matchData.players = this.formatPlayersInfo(this.matchData.players);
              this.updateRadiantPlayers(this.matchData.players.slice(0,5));
              this.updateDirePlayers(this.matchData.players.slice(5));
              this.setExpandedMatchId(this.matchId);
            }
          });
      },
      formatPlayersInfo(players) {
        console.log('format run');
        let itemId = 0;
        let prefix = 'https://api.opendota.com';
        return players.map((player) =>{
          for(let i = 0; i < 6; i++){
            itemId = player['item_' + i];
            if( itemId != 0){
              player['item_' + i] = this.items[this.itemids[itemId]];
            }
          }
          return player;
        });
      },
      updateRadiantPlayers(players){
        this.$store.dispatch('setRadiantPlayers', players);
      },
      updateDirePlayers(players){
        this.$store.dispatch('setDirePlayers', players);
      },
      setExpandedMatchId(matchId){
        this.$store.dispatch('setExpandedMatchId', matchId);
      }
    }
  }
</script>
...
.dire{
  filter:drop-shadow(0 0 5px #ff4c4c);
}
